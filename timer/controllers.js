...
function makeIcon(kind) {
  ...
  if (kind === "play") {
    ...
    svg.setAttribute("stroke", "currentColor");
    svg.setAttribute("stroke-width", "1.6");
    svg.append(svgEl("path", { "stroke-linecap": "round", "stroke-linejoin": "round", d: "M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" }));
  } else if (kind === "stop") {
...
async function multiClearAll() {
  if (!state.timers?.length) return;
  const ok = await confirmModal({
    title: t("mtClearAllTitle"),
    body: t("mtClearAllBody"),
    confirmLabel: t("mtClearAll"),
    danger: true,
  });
  if (!ok) return;
  for (const x of state.timers) bg("multi:clear", { id: x.id });
  multiAlerted.clear();
  // Wiping the list also drops the "currently loaded collection" label —
  // there's nothing left of it on the page.
  persist({ timers: [], timerCollectionLoaded: null });
  updateWakeLock();
}
...
function renderCollectionBar() {
  ...
  if (hasTimers) {
    ...
    clearAll.className = "btn btn-soft";
